...
export function getParentId(subpostId: string): string {
  ...
  if (segments.length <= 1) return subpostId

  // Handle yyyy/slug/... format (e.g., 2024/ai-town/index.md)
...
export function isSubpost(postId: string): boolean {
  const segments = postId.split('/').filter(Boolean)
  if (segments.length <= 1) return false

  // Handle yyyy/slug format as parent (not subpost)
  // e.g., 2024/ai-town/index.md has segments ['2024', 'ai-town', 'index.md']
  // This is a parent post, not a subpost
  if (YEAR_SEGMENT.test(segments[0])) {
