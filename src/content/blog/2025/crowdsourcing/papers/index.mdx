---
title: 'Crowdsourcing Papers'
description: '众包平台质量控制、真值推断、标注一致性、用户画像和异常检测论文阅读记录。'
date: 2025-01-05
tags: ['工程', '论文']
authors: ['mathew']
draft: false
...
<Callout variant="note" title="稳定高效的 DS 算法" defaultOpen={false}>
...
<Callout variant="note" title="FastDS Resources" defaultOpen={false}>
...
<Callout variant="tip" title="LA 算法的优/劣势">
...
<Callout variant="note" title="Diligent/Competent/Incompetent/Sloppy/Spammer" defaultOpen={false}>
...
<Callout variant="warning" title="部分任务 (如图片相关) 标注时间长/速度慢带来的标注结果并不一定好" defaultOpen={false}>
...
<Callout variant="note" title="定义相似度 → 聚类 → 筛选 → 剔除" defaultOpen={false}>
