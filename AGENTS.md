...
封面使用浅色背景、无标题文字的原创插画，横向构图接近 1200:630。逐篇从正文的独特细节、原理或矛盾确定意象，并选择合适的媒介、构图和配色，避免统一的几何图标模板。优先保留作品的文学艺术气质，不强制转换为 SVG。

封面存放在文章目录下的 `images/`，通过 `imageWithoutText` 引用；新生成的封面通常使用 `cover-literary.webp`，选定的 PNG 原图也可保留。若同时配置 `imageWithText`，应同步确认社交分享使用的封面。正文中的原有图片保留。每篇封面的构思和 imagegen 提示词记录在 `docs/cover-art-direction.json`。
